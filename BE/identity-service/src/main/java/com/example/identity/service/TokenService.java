...
public class TokenService {
	...
	public Jwt verifyToken(String token, Boolean isRefreshToken) {
		try {
			...
			if (Objects.isNull(tokenId) || redisService.getInvalidatedToken(tokenId) != null) {
	...
	private String buildScope(UserEntity userEntity) {
		...
		if (!CollectionUtils.isEmpty(userEntity.getRoleEntities()))
			userEntity.getRoleEntities().forEach(roleEntity -> {
				...
				if (!CollectionUtils.isEmpty(roleEntity.getPermissionEntities()))
					roleEntity.getPermissionEntities().forEach(permissionEntity -> stringJoiner.add(permissionEntity.getName()));
			});
		// Return scope
